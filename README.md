...
The landing screen is alive before any audio starts: slow aurora-borealis curtains (a pale rippling edge with a coloured veil hanging below, each breathing on its own clock) over a gently drifting starfield — deliberately ring-free and strobe-free so it can sit on a projector wall. Paused automatically while a visualizer runs.
